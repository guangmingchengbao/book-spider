...
class Z51zhySpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for i in range(1, max_page):
            yield scrapy.Request(url='{}?currentPage={}'.format(url, i), headers=self.headers, callback=self.parse)
    ...
    def parse(self, response):
        ...
        for id in ids:
            ...
            yield response.follow(url=url, meta=meta, body=body, method='POST', headers=self.headers, callbac=self.parse_authorize)
    ...
    def parse_authorize(self, response):
        ...
        yield response.follow(url=url, meta=meta, headers=self.headers, callbac=self.parse_detail)
